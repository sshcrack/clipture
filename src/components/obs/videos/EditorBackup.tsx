...
export default function Editor({ clipName, onBack }: { clipName: string, onBack: () => void }) {
    ...
    const updateElements = () => {
        ...
        const setBar = (currTime: number, ref: HTMLDivElement) => {
            ...
            ref.setAttribute("style", `display: ${currDisplay};transform: translateX(${position}px) scale(1.1)`)
        }
    ...
    useEffect(() => {
        ...
        const clientWidth = outerBar.current.clientWidth
        endBar.current.setAttribute("style", `transform: translateX(${clientWidth}px) scale(1.1)`)
    }, [outerBar, endBar])
