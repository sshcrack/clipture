...
export const MenuListItem: React.FC<MenuListItemProps> = ({ mainIndex, subIndex, curItem, style, changeCheckState }) => {
    ...
    const listItemClass = classnames("list-item", { 'selected': selected && curItem.type !== 'separator' && curItem.enabled !== false })
    const menuItemClass = classnames("menu-item", { 'disabled': curItem.type !== 'separator' && curItem.enabled === false })
    const innerContent = (
        curItem.type === 'separator' ?
            ...
            <div className={menuItemClass} onClick={handleClick}>
                ...
                {curItem.accelerator && <div className="accelerator">
                    {curItem.accelerator}
                </div>}
            </div>
    )
